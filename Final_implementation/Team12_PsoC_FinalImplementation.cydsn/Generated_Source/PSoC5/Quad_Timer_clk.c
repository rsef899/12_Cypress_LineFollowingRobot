...
/*******************************************************************************
* Function Name: Quad_Timer_clk_Start
********************************************************************************
*
* Summary:
...
void Quad_Timer_clk_Start(void) 
{
    ...
    Quad_Timer_clk_CLKEN |= Quad_Timer_clk_CLKEN_MASK;
	Quad_Timer_clk_CLKSTBY |= Quad_Timer_clk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Quad_Timer_clk_Stop
********************************************************************************
*
* Summary:
...
void Quad_Timer_clk_Stop(void) 
{
    ...
    Quad_Timer_clk_CLKEN &= (uint8)(~Quad_Timer_clk_CLKEN_MASK);
	Quad_Timer_clk_CLKSTBY &= (uint8)(~Quad_Timer_clk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Quad_Timer_clk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Quad_Timer_clk_CLKEN & Quad_Timer_clk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Quad_Timer_clk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Quad_Timer_clk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Quad_Timer_clk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Quad_Timer_clk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Quad_Timer_clk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Quad_Timer_clk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Quad_Timer_clk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Quad_Timer_clk_GetSourceRegister();
    uint16 oldDivider = Quad_Timer_clk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Quad_Timer_clk_CLKEN & Quad_Timer_clk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Quad_Timer_clk_DIV_PTR, clkDivider);
                Quad_Timer_clk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Quad_Timer_clk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Quad_Timer_clk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Quad_Timer_clk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Quad_Timer_clk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Quad_Timer_clk_DIV_PTR);
}


/*******************************************************************************
* Function Name: Quad_Timer_clk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Quad_Timer_clk_MOD_SRC |= modeBitMask & (uint8)Quad_Timer_clk_MODE_MASK;
}


/*******************************************************************************
* Function Name: Quad_Timer_clk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Quad_Timer_clk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Quad_Timer_clk_MODE_MASK));
}


/*******************************************************************************
* Function Name: Quad_Timer_clk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Quad_Timer_clk_MOD_SRC & (uint8)(Quad_Timer_clk_MODE_MASK);
}


/*******************************************************************************
* Function Name: Quad_Timer_clk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Quad_Timer_clk_GetDividerRegister();
    uint8 oldSrc = Quad_Timer_clk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Quad_Timer_clk_MOD_SRC |= CYCLK_SSS;
        Quad_Timer_clk_MOD_SRC =
            (Quad_Timer_clk_MOD_SRC & (uint8)(~Quad_Timer_clk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Quad_Timer_clk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Quad_Timer_clk_MOD_SRC & Quad_Timer_clk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Quad_Timer_clk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Quad_Timer_clk_PHASE = clkPhase & Quad_Timer_clk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Quad_Timer_clk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Quad_Timer_clk_PHASE & Quad_Timer_clk_PHASE_MASK;
}
